...
public class RealisticDronePhysics : NetworkBehaviour
{
    ...
    void Start()
    {
        ...
        // Комплексная проверка всех необходимых компонентов
        bool hasErrors = false;
        
        if (rb == null)
        {
            Debug.LogError("Drone: Missing Rigidbody component!");
            hasErrors = true;
        }
        
        Transform[] motors = { motorFL, motorFR, motorBR, motorBL };
        string[] motorNames = { "Front Left", "Front Right", "Back Right", "Back Left" };
        
        for (int i = 0; i < motors.Length; i++)
        {
            if (motors[i] == null)
            {
                Debug.LogError($"Drone: {motorNames[i]} motor transform is not assigned!");
                hasErrors = true;
            }
            else if (!motors[i].gameObject.activeInHierarchy)
            {
                Debug.LogWarning($"Drone: {motorNames[i]} motor GameObject is inactive!");
            }
        }

        if (hasErrors)
        {
            Debug.LogError("Drone physics will not work correctly until errors are fixed!");
            // Можно отключить компонент для безопасности
            enabled = false;
            return;
        }
        ...
        Debug.Log("Drone physics initialized successfully");
    }
    ...
    private void ApplyGravityAndDrag()
    {
        ...
        rb.AddForce(-rb.linearVelocity * 0.03f, ForceMode.Force);
        rb.AddTorque(-rb.angularVelocity * 0.03f, ForceMode.Force);
    }
